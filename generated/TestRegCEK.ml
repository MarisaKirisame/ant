...
let () =
  add_exp
    (fun w_7 ->
      ...
      match Word.get_value hd_0 with c_0 when c_0 = tag_cont_done -> exec_done w_7 | _ -> failwith "unreachable (0)")
    0
...
let () =
  add_exp
    (fun w_0 ->
      ...
      set_env w_0 2 (Dynarray.get w_0.state.e 0);
...
let () =
  add_exp
    (fun w_1 ->
      ...
      let x_0 = resolve w_1 (Source.E 2) in
      match Word.get_value (fst x_0) with
      | 1 (* tag_Nil *) ->
          set_env w_1 3 (Memo.from_int 0);
          set_env w_1 1 (Dynarray.get w_1.state.e 3);
          assert_env_length w_1 4;
          shrink_env w_1 2;
...
let () =
  add_exp
    (fun w_2 ->
      ...
      grow_env (* match *) w_2 2;
      assert_env_length w_2 4;
      set_env w_2 2 (Dynarray.get w_2.state.e 0);
...
let () =
  add_exp
    (fun w_4 ->
      ...
      shrink_env w_4 2;
      set_env w_4 1 (Dynarray.get w_4.state.e 3);
      assert_env_length w_4 4;
      shrink_env w_4 2;
      return_n_with w_4 2 (Dynarray.get w_4.state.e 1) (pc_to_exp (int_to_pc 0)))
    4
...
let () =
  add_exp
    (fun w_3 ->
      ...
      let x_1 = resolve w_3 (Source.E 2) in
...
let () =
  add_exp
    (fun w_5 ->
      ...
      grow_env (* return value *) w_5 1;
      assert_env_length w_5 2;
      grow_env (* match *) w_5 2;
      assert_env_length w_5 4;
      set_env w_5 2 (Dynarray.get w_5.state.e 0);
...
let () =
  add_exp
    (fun w_6 ->
      ...
      let x_2 = resolve w_6 (Source.E 2) in
      ...
      | 4 (* tag_Wrap *) ->
          ...
          set_env w_6 3 (Dynarray.get w_6.state.e 4);
          assert_env_length w_6 5;
          shrink_env w_6 1;
          set_env w_6 1 (Dynarray.get w_6.state.e 3);
          assert_env_length w_6 4;
          shrink_env w_6 2;
          return_n_with w_6 2 (Dynarray.get w_6.state.e 1) (pc_to_exp (int_to_pc 0))
