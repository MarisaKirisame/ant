...
let rec better_expr =
 fun a b ->
  ...
  if sa < sb then a else if sb < sa then b else if compare_expr a b <= 0 then a else b
  ...
  if expr_equal needle e then Found (Const 1)
    ...
    | Mul (a, b) -> (
        ...
        | Missing -> (
            ...
            match right with Found rest -> Found (Mul (a, rest)) | Missing -> Missing))
    | _ -> Missing
 ...
 fun left right ->
  ...
  | Mul (a, b) -> (
      ...
      | Missing -> (
          ...
          match fb with Found rest -> Mul (b, Add (a, rest)) | Missing -> Add (left, right)))
  | _ -> Add (left, right)

let rec append_exprs = fun xs ys -> match xs with ENil -> ys | ECons (x, rest) -> ECons (x, append_exprs rest ys)

let rec insert_expr =
 fun e xs ->
  match xs with
  | ENil -> ECons (e, ENil)
  | ECons (x, rest) -> if compare_expr e x <= 0 then ECons (e, xs) else ECons (x, insert_expr e rest)

let rec sort_exprs = fun xs -> match xs with ENil -> ENil | ECons (x, rest) -> insert_expr x (sort_exprs rest)
...
let rec compare_add_term =
 fun a b ->
  ...
  if cbase = 0 then
    ...
    if acoeff < bcoeff then 0 - 1 else if acoeff > bcoeff then 1 else compare_expr a b
  else cbase
...
let rec insert_add_term =
 fun e xs ->
  match xs with
  | ENil -> ECons (e, ENil)
  | ECons (x, rest) -> if compare_add_term e x <= 0 then ECons (e, xs) else ECons (x, insert_add_term e rest)

let rec sort_add_terms =
 fun xs -> match xs with ENil -> ENil | ECons (x, rest) -> insert_add_term x (sort_add_terms rest)

let rec reverse_exprs_aux =
 fun xs acc -> match xs with ENil -> acc | ECons (x, rest) -> reverse_exprs_aux rest (ECons (x, acc))
 ...
 fun x xs ->
  ...
  | ECons (y, rest) ->
      ...
      if expr_equal merged (Add (x, y)) then
        match pick_factored x rest with NoPick -> NoPick | Pick (term, remain) -> Pick (term, ECons (y, remain))
...
let rec normalize =
 fun e ->
  ...
  | Add (a, b) -> (
      ...
      match na with Const x -> if x = 0 then nb else normalize_add_flat na nb | _ -> normalize_add_flat na nb)
  | Mul (a, b) -> (
      let na = normalize a in
      let nb = normalize b in
      match na with
