 ...
 fun xs ->
  ...
  | ECons (x, rest) -> (
      ...
      | ECons (y, ys) ->
          ...
          else factor_adjacent (insert_expr merged ys))
...
let rec search_terms =
 fun xs ->
  ...
  | ECons (x, rest) -> (
      ...
      | Pick (merged, remain) -> search_terms (insert_expr merged remain))
...
let rec search_round =
 fun xs ->
  ...
  let factored3 = search_terms factored2 in
  let factored4 = factor_adjacent factored3 in
  let factored5 = search_terms factored4 in
  let factored6 = factor_adjacent factored5 in
  search_terms factored6

let rec normalize_add_flat =
 fun left right ->
  let terms = append_exprs (flatten_add left) (flatten_add right) in
  let sorted = sort_exprs terms in
  ...
  let rescanned = combine_like_terms (sort_exprs (flatten_add best)) in
  build_add rescanned
...
let rec search_opt =
 fun fuel e ->
  if fuel = 0 then e
    ...
    match e with
    | Add (a, b) ->
        let sa = search_opt (fuel - 1) a in
        let sb = search_opt (fuel - 1) b in
        ...
        if expr_equal final merged then merged else search_opt (fuel - 1) final
    | Mul (a, b) ->
        let sa = search_opt (fuel - 1) a in
        let sb = search_opt (fuel - 1) b in
...
let rec normalize =
 fun e ->
  ...
  | Add (a, b) ->
      ...
      normalize_add_flat na nb
  | Mul (a, b) ->
      let na = normalize a in
      let nb = normalize b in
      normalize_mul_flat na nb

let rec simplify_aux =
 fun e ->
  let next = normalize e in
  let searched = search_opt 6 next in
