...
let int_list_Cons x0 x1 : Memo.seq =
  Memo.appends [ Memo.from_constructor 2; Memo.from_int x0; x1 ]

let from_ocaml_int_list x =
  match x with Nil -> int_list_Nil | Cons (x0, x1) -> int_list_Cons x0 x1
...
let rec list_incr (x0 : seq) : seq =
  todo "meow";
  exec_cek (pc_to_exp 2) (Dynarray.of_list [ x0 ]) (Memo.from_constructor 0)
...
let () =
  add_exp (fun x ->
      todo "extend env with location, set pc";
      x)
