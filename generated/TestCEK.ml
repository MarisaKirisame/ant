...
let () = add_exp (fun w_2 -> ((assert_env_length w_2 5);match (resolve w_2 (Source.E 3)) with | None -> () | Some x0_1 -> match (resolve w_2 (Source.E 4)) with | None -> () | Some x1_1 -> ((ignore (pop_env w_2));(ignore (pop_env w_2));(push_env w_2 (Memo.from_int ((Word.to_int (fst x0_1)) + (Word.to_int (fst x1_1)))));(assert_env_length w_2 4);(push_env w_2 (Dynarray.get ((w_2.state).e) 2));(assert_env_length w_2 5);let keep_0 = (env_call w_2 [3] 1) in ((w_2.state).k <- (Memo.appends [(Memo.from_constructor 3);keep_0;((w_2.state).k)]));((w_2.state).c <- (pc_to_exp 1));(stepped w_2)))) 2
let () = add_exp (fun w_1 -> ((assert_env_length w_1 2);let last_0 = (Source.E 1) in match (resolve w_1 last_0) with | None -> () | Some x_0 -> ((ignore (pop_env w_1));
 (match Word.get_value (fst x_0) with | 1 -> ((assert_env_length w_1 1);(push_env w_1 (Memo.from_constructor 1));(assert_env_length w_1 2);(return_n w_1 2 (pc_to_exp 0)))
| 2 -> (let [x0_0; x1_0] =(Memo.splits (snd x_0)) in (push_env w_1 x0_0);(push_env w_1 x1_0);(assert_env_length w_1 3);(push_env w_1 (Dynarray.get ((w_1.state).e) 1));(assert_env_length w_1 4);(push_env w_1 (Memo.from_int 1));((w_1.state).c <- (pc_to_exp 2));(stepped w_1)))))) 3
