...
let rec int_list_append_cek_of_list = function [] -> AppendCEK.Nil | x :: xs -> AppendCEK.Cons (x, int_list_append_cek_of_list xs)
let rec int_list_append_plain_of_list = function [] -> AppendPlain.Nil | x :: xs -> AppendPlain.Cons (x, int_list_append_plain_of_list xs)
let rec int_list_insertion_sort_cek_of_list = function [] -> InsertionSortCEK.Nil | x :: xs -> InsertionSortCEK.Cons (x, int_list_insertion_sort_cek_of_list xs)
...
let rec int_list_simple_filter_plain_of_list = function [] -> SimpleFilterPlain.Nil | x :: xs -> SimpleFilterPlain.Cons (x, int_list_simple_filter_plain_of_list xs)
...
let run_case
  ...
  ~(label : string)
  ~(entry_pc : int)
  ~(cek_args : Value.value list)
  ~(tag_cont_done : int)
  ~(run_memo : State.memo -> Memo.exec_result)
  ~(run_plain : unit -> 'a)
  ~(result_of_cek : Value.value -> 'b)
  ~(result_to_string : 'b -> string) =
  (* Memo *)
  Gc.full_major ();
  let result_memo = run_memo memo in
  (* CEK *)
  Gc.full_major ();
  let _ = Profile.with_slot RunLiveCommon.eval_cek_slot (fun () ->
    Memo.exec_cek_raw
      (Memo.pc_to_exp (Common.int_to_pc entry_pc))
      (Dynarray.of_list cek_args)
...
let run_all_cases ~program_name ~populate_state ~case_fn ~warmup_fn =
  ...
  populate_state ();
  let memo = Ant.Memo.init_memo () in
  let (low_entropy_ns_memo, low_entropy_ns_cek) = case_fn memo "Low entropy" low_entropy_input (filename program_name "low_entropy") in
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~filename
        ~entry_pc:1
        ~tag_cont_done:TestCEK.tag_cont_done
        ~cek_args:[ cek_list ]
        ~run_memo:(fun memo -> TestCEK.list_incr memo cek_list)
        ~run_plain:(fun () -> ignore (TestPlain.list_incr plain_list))
        ~result_of_cek:TestCEK.to_ocaml_int_list
        ~result_to_string:list_to_string_test_cek)
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~filename
        ~entry_pc:1
        ~tag_cont_done:AppendCEK.tag_cont_done
        ...
        ~result_of_cek:AppendCEK.to_ocaml_int_list
        ~result_to_string:list_to_string_append_cek)
    ~warmup_fn:(fun memo xs ->
      let cek_list = AppendCEK.from_ocaml_int_list (int_list_append_cek_of_list xs) in
      let plain_list = int_list_append_plain_of_list xs in
      let append_value = 1 in
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~result_of_cek:InsertionSortCEK.to_ocaml_int_list
        ~result_to_string:list_to_string_insertion_sort_cek)
    ~warmup_fn:(fun memo xs ->
      let cek_list = InsertionSortCEK.from_ocaml_int_list (int_list_insertion_sort_cek_of_list xs) in
      let plain_list = int_list_insertion_sort_plain_of_list xs in
      let _ =
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~result_of_cek:MergeSortCEK.to_ocaml_int_list
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~filename
        ~entry_pc:3
        ~tag_cont_done:ReverseCEK.tag_cont_done
        ~cek_args:[ cek_list ]
        ~run_memo:(fun memo -> ReverseCEK.reverse memo cek_list)
        ~run_plain:(fun () -> ignore (ReversePlain.reverse plain_list))
        ~result_of_cek:ReverseCEK.to_ocaml_int_list
        ~result_to_string:list_to_string_reverse_cek)
    ~warmup_fn:(fun memo xs ->
      let cek_list = ReverseCEK.from_ocaml_int_list (int_list_reverse_cek_of_list xs) in
      let plain_list = int_list_reverse_plain_of_list xs in
      let _ =
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~filename
        ~entry_pc:1
        ~tag_cont_done:SimpleFilterCEK.tag_cont_done
        ~cek_args:[ cek_list ]
        ~run_memo:(fun memo -> SimpleFilterCEK.filter_pos memo cek_list)
        ~run_plain:(fun () -> ignore (SimpleFilterPlain.filter_pos plain_list))
        ~result_of_cek:SimpleFilterCEK.to_ocaml_int_list
        ~result_to_string:list_to_string_simple_filter_cek)
    ~warmup_fn:(fun memo xs ->
      let cek_list = SimpleFilterCEK.from_ocaml_int_list (int_list_simple_filter_cek_of_list xs) in
      let plain_list = int_list_simple_filter_plain_of_list xs in
      let _ =
  ...
  run_all_cases
    ...
    ~case_fn:(fun memo label xs filename ->
      ...
      run_case_then_write
        ...
        ~filename
        ~entry_pc:1
        ~tag_cont_done:PairCEK.tag_cont_done
        ~cek_args:[ cek_list ]
        ~run_memo:(fun memo -> PairCEK.pair memo cek_list)
        ~run_plain:(fun () -> ignore (PairPlain.pair plain_list))
        ~result_of_cek:PairCEK.to_ocaml_int_pair_list
        ~result_to_string:list_to_string_pair_cek)
    ~warmup_fn:(fun memo xs ->
      let cek_list = PairCEK.from_ocaml_int_list (int_list_pair_cek_of_list xs) in
      let plain_list = int_list_pair_plain_of_list xs in
      let _ =
