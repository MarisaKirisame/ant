...
(*
type ocaml_int_list = Nil | Cons of int * Words.words

let () = Words.set_constructor_degree 0 1
let word_list_Nil : Words.words = Words.appends [ Words.from_constructor 0 ]
let () = Words.set_constructor_degree 1 (-1)
let word_list_Cons x0 x1 : Words.words = Words.appends [ Words.from_constructor 1; Words.from_int x0; x1 ]
let from_ocaml_int_list = function Nil -> word_list_Nil | Cons (x0, x1) -> word_list_Cons x0 x1

let to_ocaml_int_list x =
  let h, t = Option.get (Words.list_match x) in
  match Word.get_value h with
  | 0 -> Nil
  | 1 ->
      let [ x0; x1 ] = Words.splits t in
      Cons (Word.get_value (Words.to_word x0), x1)
...
let rec loop i =
  if i < 30 then
    ...
    loop (i + 1)
*)
let run () : unit = print_endline "commented out."
(*
