...
let pp_expr =
  let fc pf ef (p, e) =
    break 1 ^^ string "|" ^^ space ^^ pf p ^^ space ^^ string "->" ^^ nest 2 @@ break 1 ^^ ef true e
  in
  let fm ef e inf = string "match" ^^ space ^^ ef false e ^^ space ^^ string "of" ^^ nest 2 @@ inf in
  ...
  let rec f c (expr : expr) =
    ...
    | Lam (xs, e) ->
        align @@ string "fun" ^^ space ^^ separate_map space pp_pattern xs ^^ space ^^ string "->" ^^ nest 2 @@ break 1
        ^^ f true e
        |> pp
    ...
    | Let (BSeq e1, e2) -> f false e1 ^^ semi ^^ break 1 ^^ f false e2
    | Let (_, _) -> failwith "Not implemented"
    | If (e, e1, e2) ->
        string "if" ^^ break 1 ^^ f true e ^^ break 1 ^^ string "then" ^^ break 1 ^^ nest 2 @@ f true e1
        ^^ string "else" ^^ break 1 ^^ f true e2
    ...
    | Match (e, SwitchInt (cases, default)) ->
        let g x = string @@ string_of_int x in
        fm f e @@ concat_map (fc g f) cases ^^ fd default f
