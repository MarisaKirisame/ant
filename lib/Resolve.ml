...
let rec resolve_expr ctx x =
  ...
  | Let (BOne (p, l, info), r, info') -> Let (BOne (p, recurse l, info), recurse r, info')
  | Ctor _ | Int _ -> x
...
let resolve_stmt ctx stmt =
  match stmt with
  | Type _ -> stmt
  | Term (BOne (p, x, info)) -> Term (BOne (p, resolve_expr ctx x, info))
  | _ -> failwith ("resolve_stmt: " ^ Syntax.string_of_document @@ Syntax.pp_stmt stmt)

let build_ctx ctx stmt =
  match stmt with
  | Type _ -> ()
  | Term (BOne (PVar (f, _), _, _)) -> Hashtbl.add_exn ~key:f ~data:() ctx.func
