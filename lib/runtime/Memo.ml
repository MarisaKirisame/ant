...
type exec_mode = Memoized | Cek
type exec_config = { mode : exec_mode; memo : memo option }
...
let choose_step_option x y =
  match (x, y) with None, y -> y | x, None -> x | Some x, Some y -> Some (choose_step_result x y)
...
let rec lookup_step_aux (x : trie option) (value : Value.value) (acc : Value.value Rev.t)
  ...
  | None -> step
  | Some trie -> (
      if step_sc step >= max_sc_of_trie trie then step
      else
        match trie with
        | Leaf { prefix; step = step_; _ } -> (
            let m = Generic.measure ~monoid:Value.monoid ~measure:Value.measure value in
            let pm = Pattern.pattern_measure prefix in
        ...
        | Branch br -> (
            ...
            | Some value ->
                ...
                let explore child child_max step =
                  if child_max = 0 || step_sc step >= child_max then step
                  else
                    match child with
                    | None -> step
                    | Some (trie, (value, acc)) -> lookup_step_aux (Some trie) value acc step
                in
                if var_max >= const_max then
                  let acc = explore var_child var_max step in
                  explore const_child const_max acc
                else
                  let acc = explore const_child const_max step in
...
let memo_config (memo : memo) : exec_config = { mode = Memoized; memo = Some memo }
let cek_config : exec_config = { mode = Cek; memo = None }
let default_exec_config () : exec_config = memo_config (init_memo ())

let exec_cek_with_config ?config (c : exp) (e : words Dynarray.t) (k : words) : exec_result =
  match config with
  | Some { mode = Cek; _ } -> exec_cek_without_memo c e k
  | Some { mode = Memoized; memo = Some memo } -> exec_cek c e k memo
  | Some { mode = Memoized; memo = None } -> exec_cek c e k (init_memo ())
  | None -> exec_cek c e k (init_memo ())
