...
let hash_slot = Profile.register_slot Profile.memo_profile "words_hash"
let hash (s : words) : Hasher.t = Profile.with_slot hash_slot (fun () -> Lazy.force (summary s).hash)
