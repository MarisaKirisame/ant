...
let app_list_full_to_list x = match x with None -> [] | Some x -> app_list_full_to_list x
let app_list_append x y = match (x, y) with None, _ -> y | _, None -> x | Some x, Some y -> Some (AppAlist (x, y))
let app_list_empty = None
let app_list_cons x xs = app_list_append (Some (LeafAlist x)) xs
let app_list_is_empty x = match x with None -> true | Some _ -> false
let rec app_list_from_list x = match x with [] -> app_list_empty | h :: t -> app_list_cons h (app_list_from_list t)
