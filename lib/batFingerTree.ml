...
(* The fingertree at batteries does not expose the finger tree adt itself,
 *   but we need to manipulate the adt directly for better performance.
 * todo: fork it properly instead of copying the file.
 *)
...
module Generic = struct
  ...
  *)
  ...
  type ('a, 'm) node = Node2 of 'm * 'a * 'a | Node3 of 'm * 'a * 'a * 'a
  ...
  let empty = Nil
  let singleton ~measure a = Single (measure a, a)
  ...
  let rec pp_debug_tree : 'a 'm. 'm printer -> 'a printer -> ('a, 'm) fg printer =
   fun pp_measure pp_a f -> function
    ...
    | Single (v, a) -> Format.fprintf f "@[<2>Single (@,%a,@ %a@])@]" pp_measure v pp_a a
    | Deep (v, pr, m, sf) ->
  ...
  let measure_t_node ~monoid = function Nil -> monoid.zero | Single (v, _) -> v | Deep (v, _, _, _) -> v
  let measure_t ~monoid ~measure:_ = function Nil -> monoid.zero | Single (v, _) -> v | Deep (v, _, _, _) -> v
  ...
  let rec check_measures :
   ...
   fun ~monoid ~measure ~eq check -> function
    | Nil -> true
    | Single (v, a) -> check a && eq (measure a) v
    | Deep (v, pr, m, sf) ->
  ...
  let rec cons_aux : 'a 'm. monoid:'m monoid -> (('a, 'm) node, 'm) fg -> ('a, 'm) node -> (('a, 'm) node, 'm) fg =
   fun ~monoid t a ->
    match t with
    | Nil -> Single (measure_node a, a)
    | Single (_, b) -> deep ~monoid (one_node a) Nil (one_node b)
  ...
  let cons ~monoid ~measure t a =
    match t with
    | Nil -> Single (measure a, a)
    | Single (_, b) -> deep ~monoid (one ~measure a) Nil (one ~measure b)
    | Deep (_, Four (_, b, c, d, e), m, sf) ->
  ...
  let rec snoc_aux : 'a 'm. monoid:'m monoid -> (('a, 'm) node, 'm) fg -> ('a, 'm) node -> (('a, 'm) node, 'm) fg =
   fun ~monoid t a ->
    match t with
    | Nil -> Single (measure_node a, a)
    | Single (_, b) -> deep ~monoid (one_node b) Nil (one_node a)
  ...
  let snoc ~monoid ~measure t a =
    match t with
    | Nil -> Single (measure a, a)
    | Single (_, b) -> deep ~monoid (one ~measure b) Nil (one ~measure a)
    | Deep (_, pr, m, Four (_, b, c, d, e)) ->
  ...
  let head_exn = function Nil -> raise Empty | Single (_, a) -> a | Deep (_, pr, _, _) -> head_digit pr
  let head = function Nil -> None | Single (_, a) -> Some a | Deep (_, pr, _, _) -> Some (head_digit pr)
  let last_exn = function Nil -> raise Empty | Single (_, a) -> a | Deep (_, _, _, sf) -> last_digit sf
  let last = function Nil -> None | Single (_, a) -> Some a | Deep (_, _, _, sf) -> Some (last_digit sf)
  ...
  let nodes =
    ...
    let rec nodes_aux ~monoid ~measure ts sf2 =
      (* no idea if this should be tail rec *)
      match (ts, sf2) with
      | [], One _ -> assert false
  ...
  let rec reverse_aux :
   ...
   fun ~monoid reverse_a -> function
    | Nil -> Nil
    | Single (_, a) ->
        let ra = reverse_a a in
        Single (measure_node ra, ra)
    | Deep (_, pr, m, sf) ->
  ...
  let split_digit ~monoid ~measure p i = function
    ...
    | Two (_, a, b) ->
        ...
        if p i' then Split ([], a, [ b ]) else Split ([ a ], b, [])
    | Three (_, a, b, c) ->
        let i' = monoid.combine i (measure a) in
        if p i' then Split ([], a, [ b; c ])
        else
          let i'' = monoid.combine i' (measure b) in
          if p i'' then Split ([ a ], b, [ c ]) else Split ([ a; b ], c, [])
    | Four (_, a, b, c, d) ->
        let i' = monoid.combine i (measure a) in
  ...
  let deep_left ~monoid ~measure pr m sf =
    ...
    | [] -> (
        ...
        | Vnil -> to_tree_digit ~monoid ~measure sf
        | Vcons (a, m') -> deep ~monoid (to_digit_node a) m' sf)
    | _ -> deep ~monoid (to_digit_list ~monoid ~measure pr) m sf
  ...
  *)
  ...
  let rec lookup_tree : 'a 'm. monoid:'m monoid -> measure:('a -> 'm) -> ('m -> bool) -> 'm -> ('a, 'm) fg -> 'm * 'a =
   fun ~monoid ~measure p i -> function
    | Nil -> raise Empty
    | Single (_, x) -> (monoid.zero, x)
    | Deep (_, pr, m, sf) ->
        let m_pr = measure_digit pr in
        let vpr = monoid.combine i m_pr in
        if p vpr then lookup_digit ~monoid ~measure p i pr
        else
  ...
  (*        classic traversals       *)
  (*---------------------------------*)
  let iter f t = fold_left (fun () elt -> f elt) () t
  let iter_right f t = fold_right (fun () elt -> f elt) () t
  ...
  (* no defined because many local variables are
  ...
  let compare cmp t1 t2 =
    let rec loop cmp iter1 iter2 =
      ...
      | Some (e1, iter1), Some (e2, iter2) ->
          let c = cmp e1 e2 in
          if c <> 0 then c else loop cmp iter1 iter2
    in
    loop cmp (to_iter t1 End) (to_iter t2 End)
  ...
  let equal eq t1 t2 =
    let rec loop eq iter1 iter2 =
      ...
      | Some _, None -> false
      | None, Some _ -> false
      | Some (e1, iter1), Some (e2, iter2) -> eq e1 e2 && loop eq iter1 iter2
    in
    loop eq (to_iter t1 End) (to_iter t2 End)
...
let nat_plus_monoid = { zero = 0; combine = ( + ) }
...
let singleton x = Generic.singleton ~measure:size_measurer x
(*$T singleton
...
let set t i v =
  if i < 0 || i >= size t then invalid_arg "FingerTree.set: Index out of bounds";
...
let invariants t = assert (check_measures t)
